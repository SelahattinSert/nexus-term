...
export default function ErrorOverlay({ sessionId }) {
  ...
  return (
    <div className="absolute top-4 right-4 z-50 flex flex-col gap-2 max-w-sm pointer-events-none">
      {errors.map((error) => (
        <div key={error.id} className="bg-ctp-base border border-ctp-red rounded-md shadow-lg p-3 pointer-events-auto flex flex-col animate-in fade-in slide-in-from-top-4">
          <div className="flex justify-between items-start mb-1">
            <div className="flex items-center gap-2 text-ctp-red font-bold">
              <AlertTriangle size={16} />
              <span>{error.title}</span>
            </div>
            <button onClick={() => dismissError(error.id)} className="text-ctp-subtext0 hover:text-ctp-text">
              <X size={16} />
            </button>
          </div>
          <p className="text-ctp-text text-sm mb-3">{error.description}</p>
            ...
            <div className="flex flex-col gap-2">
              {error.actions.map((action, idx) => (
                <button
                  ...
                  className="flex items-center justify-center gap-2 bg-ctp-surface0 hover:bg-ctp-surface1 text-ctp-text text-sm py-1.5 px-3 rounded transition-colors"
                >
